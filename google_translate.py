...
class MyWindow(QMainWindow, Ui_MWin):
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        try:
            with open('./source/style.qss') as f:
                style = f.read()  # 读取样式表
    ...
    def connectSlots(self):
        ...
        self.about.triggered.connect(lambda: QDesktopServices.openUrl(
            QUrl('https://github.com/LewisTian/PyQt5-Apps/tree/PyQt5-Apps/google-translate')))
        self.about.setShortcut("CTRL+H")
        self.donate.triggered.connect(
            lambda: QDesktopServices.openUrl(QUrl('https://lewistian.github.io/2018/01/01/donate/')))
        self.reportBug.triggered.connect(
            lambda: QDesktopServices.openUrl(QUrl('https://github.com/LewisTian/PyQt5-Apps/issues')))
    ...
    def openFileSlot(self):
        ...
        if filename:
            ...
            with open(filename, encoding='utf-8') as f:
                try:
                    self.originText.setPlainText(self, str(f.read()))
    ...
    def transTextToZhCN(self):
        ...
        if text:
            if self.paperMode.isChecked():  # if paper mode is true, line breaks will re replaced by blanks
                ...
                text = re.sub(r'', '', text)
                # add on 19/05/16
                text = text.replace('', 'fi').replace('', 'ffi').replace('', 'ff').replace('', 'fl').replace('',
                                                                                                                 'th').replace(
                    '', 'ft').replace('', 'ft').replace('', 'tt')
    ...
    def onClipboradChanged(self):
        ...
        if text and self.isRealTimeTrans:
            content = str(text)
            # print(content)
            content = content.replace('', 'fi').replace('', 'ffi').replace('', 'ff').replace('', 'fl').replace('',
                                                                                                                   'th').replace(
                '', 'ft').replace('', 'ft').replace('', 'tt')
            if self.paperMode.isChecked():  # if paper mode is true, line breaks will re replaced by blanks
                content = re.sub(r'\n|\s+', ' ', content)
